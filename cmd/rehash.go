...
var rehashCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !slices.Contains([]string{"sha1", "sha512", "sha256"}, args[0]) {
			ui.Error.Printf("Hash format '%s' is not supported\n", args[0])
			os.Exit(1)
		}
		...
		err = index.Refresh()
		if err != nil {
			ui.Error.Printf("Error refreshing index: %v\n", err)
			os.Exit(1)
		}

		err = index.Write()
		if err != nil {
			ui.Error.Printf("Error writing index: %v\n", err)
			os.Exit(1)
		}

		err = pack.UpdateIndexHash()
		if err != nil {
			ui.Error.Printf("Error updating index hash: %v\n", err)
			os.Exit(1)
		}

		err = pack.Write()
		if err != nil {
			ui.Error.Printf("Error writing pack: %v\n", err)
			os.Exit(1)
		}
	},
}
