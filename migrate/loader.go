...
var loaderCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(currentLoaders) == 0 {
			ui.Error.Println("No loader is currently set in your pack.toml!")
			os.Exit(1)
		} else if len(currentLoaders) > 1 {
			ui.Error.Println("You have multiple loaders set in your pack.toml, this is not supported!")
			os.Exit(1)
		}
		// Get the Minecraft version for the pack
		mcVersion, err := modpack.GetMCVersion()
		if err != nil {
			ui.Error.Printf("Error getting Minecraft version: %s\n", err)
			os.Exit(1)
		}
		if args[0] == "latest" || args[0] == "recommended" {
			ui.Muted.Printf("Updating to %s loader version\n", args[0])
			...
			ui.Muted.Println("Updating to explicit loader version")
			// This one is easy :D
...
func validateVersion(versions []string, version string, gottenLoader core.ModLoaderComponent) {
	if !slices.Contains(versions, version) {
		ui.Error.Printf("Version %s is not a valid version for %s\n", version, gottenLoader.FriendlyName)
		os.Exit(1)
	}
}
...
func updatePackToVersion(version string, modpack core.Pack, loader core.ModLoaderComponent) bool {
	...
	if version == modpack.Versions[loader.Name] {
		ui.Info.Printf("%s is already on version %s!\n", loader.FriendlyName, version)
		return false
	}
	// Set the latest version
	modpack.Versions[loader.Name] = version
	ui.Success.Printf("Updated %s to version %s\n", ui.Bold.Sprint(loader.FriendlyName), ui.Bold.Sprint(version))
	return true
}
